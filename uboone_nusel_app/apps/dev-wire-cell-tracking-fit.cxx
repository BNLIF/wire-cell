...
int main(int argc, char* argv[])
{
  ...
  mp.set_time_offset(time_offset);
  ...
  //
   ...
   //   FlashTPCBundleSelection matched_bundles = WCP2dToy::tpc_light_match(time_offset,nrebin,group_clusters,flashes);
   ...
   for (size_t i=0;i!=live_clusters.size();i++){
     ...
     live_clusters.at(i)->dQ_dx_fit(global_wc_map, 4*units::microsecond);
   ...
   for (auto it = group_clusters.begin(); it!= group_clusters.end(); it++){
     //     FlashTPCBundle *bundle = *it;
     PR3DCluster *main_cluster = it->first;//bundle->get_main_cluster();//std::get<0>(*it);
  ...
  for (auto it = group_clusters.begin(); it!= group_clusters.end(); it++){
    // FlashTPCBundle *bundle = *it;
    PR3DCluster *main_cluster = it->first;//bundle->get_main_cluster();
    //    Opflash *flash = bundle->get_flash();
    //  if (flash!=0){
