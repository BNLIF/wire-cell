...
{
  if (argc < 3) {
    cerr << "usage: wire-cell-uboone /path/to/ChannelWireGeometry.txt /path/to/celltree.root eve_num event_no -t[0,1] -s[0,1,2] -f[0,1]" << endl;
    return 1;
  }
  ...
  int shortedU = 0; // if 1 do charge correction
...
if(beamspill || beam==-1){  
  ...
  for (size_t i=0;i!=calibGaussian_channelId->size();i++){
  //  int chid = calibWiener_channelId->at(i);
    int chid = calibGaussian_channelId->at(i);
    //TH1F *htemp = (TH1F*)calibWiener_wf->At(i);
    TH1F *htemp = (TH1F*)calibGaussian_wf->At(i);
    if (chid < nwire_u){
  ...
  for (int i=start_num;i!=end_num+1;i++){
    ...
    //   WCP::GeomCellMap cell_wires_map1 = lowmemtiling[i]->get_cell_wires_map();
    //   WCP::GeomCellMap cell_wires_map2 = lowmemtiling[i-1]->get_cell_wires_map();
...
if (no_dead_channel==1){
  ...
  //   TGraph2D *g1 = new TGraph2D();
	...
	if (nq>0){
	  ...
	  x = mcell->GetTimeSlice()*nrebin/2.*unit_dis/10. - frame_length/2.*unit_dis/10.;
       ...
       double center_U = offset_u + (slope_yu*p.y + slope_zu*p.z);
  ...
  if (save_file==2){
	...
	q->push_back(chargesolver[time_slice_temp]->get_mcell_charge(mcell)*correction);
	...
	for (auto it1 = cell_wires_map[mcell].begin(); it1!= cell_wires_map[mcell].end(); it1++){
	  MergeGeomWire *mwire = (MergeGeomWire*)(*it1);
	  if (mwire->get_allwire().front()->iplane()==0){
	    uq->push_back(wire_charge[mwire]*correction);
	    udq->push_back(wire_charge_error[mwire]);
	  }else if(mwire->get_allwire().front()->iplane()==1){
	    vq->push_back(wire_charge[mwire]*correction);
	    vdq->push_back(wire_charge_error[mwire]);
	  }else if(mwire->get_allwire().front()->iplane()==2){
	    wq->push_back(wire_charge[mwire]*correction);
