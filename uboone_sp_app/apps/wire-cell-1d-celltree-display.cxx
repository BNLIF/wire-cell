...
{
  if (argc < 4) {
    cerr << "usage: wire-cell-1d-celltree-display /path/to/ChannelWireGeometry.txt /path/to/celltree.root eve_num " << endl;
    return 1;
  }
  ...
  std::vector<float> uplane_rms(2400,0.01);
  std::vector<float> vplane_rms(2400,0.01);
  std::vector<float> wplane_rms(3256,0.01);
  ...
  const int nwindow_size = ((TH1F*)calibGaussian_wf->At(0))->GetNbinsX();
  ...
  for (size_t i=0;i!=calibGaussian_channelId->size();i++){
    ...
    //std::cout << chid << " " << htemp->GetBinContent(100) << std::endl;
